...
const initialBillerProducts = [
  {
    ...
    disturbance_level: '',
    additional_info: '',
    created_at: '2025-12-10 07:17:53',
    currency: 'IDR',
  },
  {
...
const BILLER_CODES = ['lapak_gaming', 'rbxchest']
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-2xl w-full max-w-sm mx-4 overflow-hidden border border-neutral-200">
        <div className="p-6">
          ...
          <h3 className="text-lg font-semibold text-neutral-800 text-center mb-2">Delete Biller Product</h3>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={onConfirm}
              className="flex-1 px-4 py-2.5 bg-red-600 text-white rounded-md font-medium hover:bg-red-700 transition-all"
            >
              Delete
            </button>
          </div>
...
function BulkUploadModal({ isOpen, onClose, onUpload, type }) {
  ...
  const templateUrl = type === 'upload' 
    ? '/templates/biller-product-bulk-upload-template.xlsx' 
    : '/templates/biller-product-bulk-edit-template.xlsx'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-2xl w-full max-w-md mx-4 overflow-hidden border border-neutral-200">
        ...
        <div className="bg-neutral-800 px-6 py-4">
          <h2 className="text-xl font-semibold text-neutral-100">
            {type === 'upload' ? 'Bulk Create Biller Products' : 'Bulk Update Biller Products'}
          </h2>
        </div>
...
function BillerProduct() {
  const [products, setProducts] = useState(initialBillerProducts)
  ...
  const clearFilters = () => {
    setFilters({ id: '', biller_code: '', biller_product_code: '', status: '' })
  }
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (filters.biller_code) {
      result = result.filter(p => p.biller_code.toLowerCase().includes(filters.biller_code.toLowerCase()))
    }
    if (filters.biller_product_code) {
      result = result.filter(p => p.biller_product_code.toLowerCase().includes(filters.biller_product_code.toLowerCase()))
    }
    if (filters.status) {
  ...
  const handleBulkUpload = (file) => {
    // In a real app, this would parse the file and create biller products
    console.log('Bulk upload file:', file.name)
    alert(`File "${file.name}" uploaded successfully! In a real app, this would process the file and create biller products.`)
  }

  const handleBulkEdit = (file) => {
    // In a real app, this would parse the file and update biller products
    console.log('Bulk edit file:', file.name)
    alert(`File "${file.name}" uploaded successfully! In a real app, this would process the file and update biller products.`)
  }

  const hasActiveFilters = filters.id || filters.biller_code || filters.biller_product_code || filters.status
  ...
  return (
    <div className="min-h-screen bg-neutral-100">
      ...
      <div className="bg-white border-b border-neutral-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold text-neutral-800">Biller Products / Server</h1>
          </div>
      ...
      <div className="px-6 pt-6">
        <div className="bg-white rounded-lg shadow-sm border border-neutral-200 p-4 mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-neutral-500 mb-1">Biller Code</label>
              <input
                type="text"
                name="biller_code"
                value={filters.biller_code}
                onChange={handleFilterChange}
                placeholder="Search by biller code..."
                className="w-full px-3 py-2 bg-neutral-50 border border-neutral-200 rounded-md text-sm text-neutral-800 placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-neutral-300 focus:border-neutral-300 transition-all"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-neutral-500 mb-1">Biller Product Code</label>
              <input
                type="text"
                name="biller_product_code"
                value={filters.biller_product_code}
                onChange={handleFilterChange}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-neutral-50 border-b border-neutral-200">
                  ...
                  <th className="text-left px-4 py-3 font-medium text-neutral-600">additional_info</th>
                ...
                {filteredAndSortedProducts.length === 0 ? (
                  ...
                  filteredAndSortedProducts.map((product, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-neutral-400">{product.additional_info || '-'}</td>
      ...
      <DeleteConfirmModal
        ...
        productName={deletingProduct?.biller_product_code}
      />
